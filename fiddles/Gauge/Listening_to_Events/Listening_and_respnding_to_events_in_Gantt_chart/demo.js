            ...
            "dataplotClick" : function (evtObj, argObj){
                ...
                var getNumOfDays = function(startDt,endDate){
                    ...
                    getDaysTimeInvolved = function(dataArray){
                        var startDate = new Date(dataArray[0].start), endDate = new Date(dataArray[0].end), currStartDate, currEndDate, totalDays;
                        
                        for (var j = 0; j < dataArray.length; j++){
                            currStartDate = new Date(dataArray[j].start);
                            currEndDate = new Date(dataArray[j].end);
                            (currStartDate.getTime() < startDate.getTime()) ? 
                                startDate = currStartDate : "";
                            (currEndDate.getTime() > endDate.getTime()) ? 
                                endDate = currEndDate : "";  
                        }
                ...
                annotations && annotations.update('infoText', {"text" : infoText});
            }
        }
    }).render();
});